...
namespace TaskAssigmentApp.Domain.Services
{
  public class TicketAssignmentWeeklyCheckService : ITicketAssignmentCheckService
  {
    ...
    {
     var tickets = _ticketRepository.WhereAsync(x => x.EmployeeId == emp.Id && x.StartDate.Date >= DateTime.Now.Date && x.EndDate.Date <= DateTime.Now.AddDays(7).Date).GetAwaiter().GetResult();

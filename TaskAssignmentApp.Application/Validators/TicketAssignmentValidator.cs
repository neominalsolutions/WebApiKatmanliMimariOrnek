  ...
  public class TicketAssignmentValidator:AbstractValidator<TicketAssignmentRequestDto>
  {
    ...
    public TicketAssignmentValidator()
    {
      RuleFor(x => x.Description)
        .MaximumLength(200)
        .WithMessage("Ticket Description alanı 200 karakter ile sınırlıdır")
        .NotNull().NotEmpty()
        .WithMessage("Ticket Description alanı zorunlu bir alandır boş geçilemez");
      ...
      //RuleFor(x => x).Must(ObjectCheck).WithMessage("Geçmiş tarihli bir görev tanımı yapılamaz");

    }


    //private bool ObjectCheck(TicketAssignmentRequestDto dto)
    //{
